...
def main(args):
    ...
    ignore_gripper_safety_check = (
        args.agent == "gello" and not args.bimanual and robot_client.num_dofs() == 7
    )
    ...
    if ignore_gripper_safety_check:
        abs_deltas[-1] = 0.0
    ...
    action_deltas = action - joints
    if ignore_gripper_safety_check:
        action_deltas[-1] = 0.0
    print(action_deltas)
    if (action_deltas > action_threshold).any(): # action delta threshold
        print("Action is too big")

        # print which joints are too big
        joint_index = np.where(action_deltas > action_threshold)[0]
        for j in joint_index:
            print(
                f"Joint [{j}], leader: {action[j]}, follower: {joints[j]}, diff: {action_deltas[j]}"
            )
        exit()
    ...
    print("reached run control loop")
