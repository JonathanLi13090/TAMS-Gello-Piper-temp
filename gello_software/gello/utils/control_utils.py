...
def run_control_loop(
    ...
    def format_joint_positions(joints: np.ndarray) -> str:
    ...
    while True:
        ...
        if print_timing:
            ...
            status = f"Time passed: {round(num, 2)} | "
        else:
            status = ""

        status += (
            f"Leader joint positions: {format_joint_positions(action)} | "
            f"Follower joint positions: {format_joint_positions(follower_joints)}"
        )
        ...
        if colors_available:
            status = colored(status, color="white", attrs=["bold"])
