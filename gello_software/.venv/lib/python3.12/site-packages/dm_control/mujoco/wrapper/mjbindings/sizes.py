"""Automatically generated by binding_generator.py.

MuJoCo header version: 3010000
"""

# pylint: disable=bad-continuation
# pylint: disable=line-too-long

array_sizes = (
{'mjdata': {'M': ('nC',),
            'act': ('na',),
            'act_dot': ('na',),
            'actuator_force': ('nu',),
            'actuator_length': ('nu',),
            'actuator_moment': ('nJmom',),
            'actuator_velocity': ('nu',),
            'body_awake': ('nbody',),
            'body_awake_ind': ('nbody',),
            'bvh_aabb_dyn': ('nbvhdynamic', 6),
            'bvh_active': ('nbvh',),
            'cacc': ('nbody', 6),
            'cam_xmat': ('ncam', 9),
            'cam_xpos': ('ncam', 3),
            'cdof': ('nv', 6),
            'cdof_dot': ('nv', 6),
            'cfrc_ext': ('nbody', 6),
            'cfrc_int': ('nbody', 6),
            'cinert': ('nbody', 10),
            'crb': ('nbody', 10),
            'ctrl': ('nu',),
            'cvel': ('nbody', 6),
            'dof_awake_ind': ('nv',),
            'eq_active': ('neq',),
            'flexedge_J': ('nJfe',),
            'flexedge_length': ('nflexedge',),
            'flexedge_velocity': ('nflexedge',),
            'flexelem_aabb': ('nflexelem', 6),
            'flexvert_J': ('nJfv', 2),
            'flexvert_length': ('nflexvert', 2),
            'flexvert_xpos': ('nflexvert', 3),
            'geom_xmat': ('ngeom', 9),
            'geom_xpos': ('ngeom', 3),
            'history': ('nhistory',),
            'light_xdir': ('nlight', 3),
            'light_xpos': ('nlight', 3),
            'mocap_pos': ('nmocap', 3),
            'mocap_quat': ('nmocap', 4),
            'moment_colind': ('nJmom',),
            'moment_rowadr': ('nu',),
            'moment_rownnz': ('nu',),
            'parent_awake_ind': ('nbody',),
            'plugin': ('nplugin',),
            'plugin_data': ('nplugin',),
            'plugin_state': ('npluginstate',),
            'qDeriv': ('nD',),
            'qH': ('nC',),
            'qHDiagInv': ('nv',),
            'qLD': ('nC',),
            'qLDiagInv': ('nv',),
            'qLU': ('nD',),
            'qM': ('nM',),
            'qacc': ('nv',),
            'qacc_smooth': ('nv',),
            'qacc_warmstart': ('nv',),
            'qfrc_actuator': ('nv',),
            'qfrc_applied': ('nv',),
            'qfrc_bias': ('nv',),
            'qfrc_constraint': ('nv',),
            'qfrc_damper': ('nv',),
            'qfrc_fluid': ('nv',),
            'qfrc_gravcomp': ('nv',),
            'qfrc_inverse': ('nv',),
            'qfrc_passive': ('nv',),
            'qfrc_smooth': ('nv',),
            'qfrc_spring': ('nv',),
            'qpos': ('nq',),
            'qvel': ('nv',),
            'sensordata': ('nsensordata',),
            'site_xmat': ('nsite', 9),
            'site_xpos': ('nsite', 3),
            'subtree_angmom': ('nbody', 3),
            'subtree_com': ('nbody', 3),
            'subtree_linvel': ('nbody', 3),
            'ten_J': ('nJten',),
            'ten_length': ('ntendon',),
            'ten_velocity': ('ntendon',),
            'ten_wrapadr': ('ntendon',),
            'ten_wrapnum': ('ntendon',),
            'tree_asleep': ('ntree',),
            'tree_awake': ('ntree',),
            'userdata': ('nuserdata',),
            'wrap_obj': ('nwrap', 2),
            'wrap_xpos': ('nwrap', 6),
            'xanchor': ('njnt', 3),
            'xaxis': ('njnt', 3),
            'xfrc_applied': ('nbody', 6),
            'ximat': ('nbody', 9),
            'xipos': ('nbody', 3),
            'xmat': ('nbody', 9),
            'xpos': ('nbody', 3),
            'xquat': ('nbody', 4)},
 'mjmodel': {'B_colind': ('nB',),
             'B_rowadr': ('nbody',),
             'B_rownnz': ('nbody',),
             'D_colind': ('nD',),
             'D_diag': ('nv',),
             'D_rowadr': ('nv',),
             'D_rownnz': ('nv',),
             'M_colind': ('nC',),
             'M_rowadr': ('nv',),
             'M_rownnz': ('nv',),
             'actuator_acc0': ('nu',),
             'actuator_actadr': ('nu',),
             'actuator_actearly': ('nu',),
             'actuator_actlimited': ('nu',),
             'actuator_actnum': ('nu',),
             'actuator_actrange': ('nu', 2),
             'actuator_armature': ('nu',),
             'actuator_biasprm': ('nu', 10),
             'actuator_biastype': ('nu',),
             'actuator_cranklength': ('nu',),
             'actuator_ctrllimited': ('nu',),
             'actuator_ctrlrange': ('nu', 2),
             'actuator_damping': ('nu',),
             'actuator_dampingpoly': ('nu', 2),
             'actuator_delay': ('nu',),
             'actuator_dynprm': ('nu', 10),
             'actuator_dyntype': ('nu',),
             'actuator_forcelimited': ('nu',),
             'actuator_forcerange': ('nu', 2),
             'actuator_gainprm': ('nu', 10),
             'actuator_gaintype': ('nu',),
             'actuator_gear': ('nu', 6),
             'actuator_group': ('nu',),
             'actuator_history': ('nu', 2),
             'actuator_historyadr': ('nu',),
             'actuator_length0': ('nu',),
             'actuator_lengthrange': ('nu', 2),
             'actuator_plugin': ('nu',),
             'actuator_trnid': ('nu', 2),
             'actuator_trntype': ('nu',),
             'actuator_user': ('nu', 'nuser_actuator'),
             'body_bvhadr': ('nbody',),
             'body_bvhnum': ('nbody',),
             'body_conaffinity': ('nbody',),
             'body_contype': ('nbody',),
             'body_dofadr': ('nbody',),
             'body_dofnum': ('nbody',),
             'body_geomadr': ('nbody',),
             'body_geomnum': ('nbody',),
             'body_gravcomp': ('nbody',),
             'body_inertia': ('nbody', 3),
             'body_invweight0': ('nbody', 2),
             'body_ipos': ('nbody', 3),
             'body_iquat': ('nbody', 4),
             'body_jntadr': ('nbody',),
             'body_jntnum': ('nbody',),
             'body_margin': ('nbody',),
             'body_mass': ('nbody',),
             'body_mocapid': ('nbody',),
             'body_parentid': ('nbody',),
             'body_plugin': ('nbody',),
             'body_pos': ('nbody', 3),
             'body_quat': ('nbody', 4),
             'body_rootid': ('nbody',),
             'body_sameframe': ('nbody',),
             'body_simple': ('nbody',),
             'body_subtreemass': ('nbody',),
             'body_treeid': ('nbody',),
             'body_user': ('nbody', 'nuser_body'),
             'body_weldid': ('nbody',),
             'bvh_aabb': ('nbvhstatic', 6),
             'bvh_child': ('nbvh', 2),
             'bvh_depth': ('nbvh',),
             'bvh_nodeid': ('nbvh',),
             'cam_bodyid': ('ncam',),
             'cam_fovy': ('ncam',),
             'cam_intrinsic': ('ncam', 4),
             'cam_ipd': ('ncam',),
             'cam_mat0': ('ncam', 9),
             'cam_mode': ('ncam',),
             'cam_output': ('ncam',),
             'cam_pos': ('ncam', 3),
             'cam_pos0': ('ncam', 3),
             'cam_poscom0': ('ncam', 3),
             'cam_projection': ('ncam',),
             'cam_quat': ('ncam', 4),
             'cam_resolution': ('ncam', 2),
             'cam_sensorsize': ('ncam', 2),
             'cam_targetbodyid': ('ncam',),
             'cam_user': ('ncam', 'nuser_cam'),
             'dof_M0': ('nv',),
             'dof_Madr': ('nv',),
             'dof_armature': ('nv',),
             'dof_bodyid': ('nv',),
             'dof_damping': ('nv',),
             'dof_dampingpoly': ('nv', 2),
             'dof_frictionloss': ('nv',),
             'dof_invweight0': ('nv',),
             'dof_jntid': ('nv',),
             'dof_length': ('nv',),
             'dof_parentid': ('nv',),
             'dof_simplenum': ('nv',),
             'dof_solimp': ('nv', 5),
             'dof_solref': ('nv', 2),
             'dof_treeid': ('nv',),
             'eq_active0': ('neq',),
             'eq_data': ('neq', 11),
             'eq_obj1id': ('neq',),
             'eq_obj2id': ('neq',),
             'eq_objtype': ('neq',),
             'eq_solimp': ('neq', 5),
             'eq_solref': ('neq', 2),
             'eq_type': ('neq',),
             'exclude_signature': ('nexclude',),
             'flex_activelayers': ('nflex',),
             'flex_bending': ('nflexbending',),
             'flex_bendingadr': ('nflex',),
             'flex_bvhadr': ('nflex',),
             'flex_bvhnum': ('nflex',),
             'flex_cellnum': ('nflex', 3),
             'flex_centered': ('nflex',),
             'flex_conaffinity': ('nflex',),
             'flex_condim': ('nflex',),
             'flex_contype': ('nflex',),
             'flex_damping': ('nflex',),
             'flex_dim': ('nflex',),
             'flex_edge': ('nflexedge', 2),
             'flex_edgeadr': ('nflex',),
             'flex_edgedamping': ('nflex',),
             'flex_edgeequality': ('nflex',),
             'flex_edgeflap': ('nflexedge', 2),
             'flex_edgenum': ('nflex',),
             'flex_edgestiffness': ('nflex',),
             'flex_elem': ('nflexelemdata',),
             'flex_elemadr': ('nflex',),
             'flex_elemdataadr': ('nflex',),
             'flex_elemedge': ('nflexelemedge',),
             'flex_elemedgeadr': ('nflex',),
             'flex_elemlayer': ('nflexelem',),
             'flex_elemnum': ('nflex',),
             'flex_elemtexcoord': ('nflexelemdata',),
             'flex_evpair': ('nflexevpair', 2),
             'flex_evpairadr': ('nflex',),
             'flex_evpairnum': ('nflex',),
             'flex_flatskin': ('nflex',),
             'flex_friction': ('nflex', 3),
             'flex_gap': ('nflex',),
             'flex_group': ('nflex',),
             'flex_internal': ('nflex',),
             'flex_interp': ('nflex',),
             'flex_margin': ('nflex',),
             'flex_matid': ('nflex',),
             'flex_node': ('nflexnode', 3),
             'flex_node0': ('nflexnode', 3),
             'flex_nodeadr': ('nflex',),
             'flex_nodebodyid': ('nflexnode',),
             'flex_nodenum': ('nflex',),
             'flex_passive': ('nflex',),
             'flex_priority': ('nflex',),
             'flex_radius': ('nflex',),
             'flex_rgba': ('nflex', 4),
             'flex_rigid': ('nflex',),
             'flex_selfcollide': ('nflex',),
             'flex_shell': ('nflexshelldata',),
             'flex_shelldataadr': ('nflex',),
             'flex_shellnum': ('nflex',),
             'flex_size': ('nflex', 3),
             'flex_solimp': ('nflex', 5),
             'flex_solmix': ('nflex',),
             'flex_solref': ('nflex', 2),
             'flex_stiffness': ('nflexstiffness',),
             'flex_stiffnessadr': ('nflex',),
             'flex_texcoord': ('nflextexcoord', 2),
             'flex_texcoordadr': ('nflex',),
             'flex_vert': ('nflexvert', 3),
             'flex_vert0': ('nflexvert', 3),
             'flex_vertadr': ('nflex',),
             'flex_vertbodyid': ('nflexvert',),
             'flex_vertedge': ('nflexedge', 2),
             'flex_vertedgeadr': ('nflexvert',),
             'flex_vertedgenum': ('nflexvert',),
             'flex_vertmetric': ('nflexvert', 4),
             'flex_vertnum': ('nflex',),
             'flexedge_J_colind': ('nJfe',),
             'flexedge_J_rowadr': ('nflexedge',),
             'flexedge_J_rownnz': ('nflexedge',),
             'flexedge_invweight0': ('nflexedge',),
             'flexedge_length0': ('nflexedge',),
             'flexedge_rigid': ('nflexedge',),
             'flexvert_J_colind': ('nJfv', 2),
             'flexvert_J_rowadr': ('nflexvert', 2),
             'flexvert_J_rownnz': ('nflexvert', 2),
             'geom_aabb': ('ngeom', 6),
             'geom_bodyid': ('ngeom',),
             'geom_conaffinity': ('ngeom',),
             'geom_condim': ('ngeom',),
             'geom_contype': ('ngeom',),
             'geom_dataid': ('ngeom',),
             'geom_fluid': ('ngeom', 12),
             'geom_friction': ('ngeom', 3),
             'geom_gap': ('ngeom',),
             'geom_group': ('ngeom',),
             'geom_margin': ('ngeom',),
             'geom_matid': ('ngeom',),
             'geom_plugin': ('ngeom',),
             'geom_pos': ('ngeom', 3),
             'geom_priority': ('ngeom',),
             'geom_quat': ('ngeom', 4),
             'geom_rbound': ('ngeom',),
             'geom_rgba': ('ngeom', 4),
             'geom_sameframe': ('ngeom',),
             'geom_size': ('ngeom', 3),
             'geom_solimp': ('ngeom', 5),
             'geom_solmix': ('ngeom',),
             'geom_solref': ('ngeom', 2),
             'geom_type': ('ngeom',),
             'geom_user': ('ngeom', 'nuser_geom'),
             'hfield_adr': ('nhfield',),
             'hfield_data': ('nhfielddata',),
             'hfield_ncol': ('nhfield',),
             'hfield_nrow': ('nhfield',),
             'hfield_pathadr': ('nhfield',),
             'hfield_size': ('nhfield', 4),
             'jnt_actfrclimited': ('njnt',),
             'jnt_actfrcrange': ('njnt', 2),
             'jnt_actgravcomp': ('njnt',),
             'jnt_actuatorid': ('njnt',),
             'jnt_axis': ('njnt', 3),
             'jnt_bodyid': ('njnt',),
             'jnt_dofadr': ('njnt',),
             'jnt_group': ('njnt',),
             'jnt_limited': ('njnt',),
             'jnt_margin': ('njnt',),
             'jnt_pos': ('njnt', 3),
             'jnt_qposadr': ('njnt',),
             'jnt_range': ('njnt', 2),
             'jnt_solimp': ('njnt', 5),
             'jnt_solref': ('njnt', 2),
             'jnt_stiffness': ('njnt',),
             'jnt_stiffnesspoly': ('njnt', 2),
             'jnt_type': ('njnt',),
             'jnt_user': ('njnt', 'nuser_jnt'),
             'key_act': ('nkey', 'na'),
             'key_ctrl': ('nkey', 'nu'),
             'key_mpos': ('nkey', ('nmocap', 3)),
             'key_mquat': ('nkey', ('nmocap', 4)),
             'key_qpos': ('nkey', 'nq'),
             'key_qvel': ('nkey', 'nv'),
             'key_time': ('nkey',),
             'light_active': ('nlight',),
             'light_ambient': ('nlight', 3),
             'light_attenuation': ('nlight', 3),
             'light_bodyid': ('nlight',),
             'light_bulbradius': ('nlight',),
             'light_castshadow': ('nlight',),
             'light_cutoff': ('nlight',),
             'light_diffuse': ('nlight', 3),
             'light_dir': ('nlight', 3),
             'light_dir0': ('nlight', 3),
             'light_exponent': ('nlight',),
             'light_intensity': ('nlight',),
             'light_mode': ('nlight',),
             'light_pos': ('nlight', 3),
             'light_pos0': ('nlight', 3),
             'light_poscom0': ('nlight', 3),
             'light_range': ('nlight',),
             'light_specular': ('nlight', 3),
             'light_targetbodyid': ('nlight',),
             'light_texid': ('nlight',),
             'light_type': ('nlight',),
             'mapD2M': ('nC',),
             'mapM2D': ('nD',),
             'mapM2M': ('nC',),
             'mat_emission': ('nmat',),
             'mat_metallic': ('nmat',),
             'mat_reflectance': ('nmat',),
             'mat_rgba': ('nmat', 4),
             'mat_roughness': ('nmat',),
             'mat_shininess': ('nmat',),
             'mat_specular': ('nmat',),
             'mat_texid': ('nmat', 10),
             'mat_texrepeat': ('nmat', 2),
             'mat_texuniform': ('nmat',),
             'mesh_bvhadr': ('nmesh',),
             'mesh_bvhnum': ('nmesh',),
             'mesh_face': ('nmeshface', 3),
             'mesh_faceadr': ('nmesh',),
             'mesh_facenormal': ('nmeshface', 3),
             'mesh_facenum': ('nmesh',),
             'mesh_facetexcoord': ('nmeshface', 3),
             'mesh_graph': ('nmeshgraph',),
             'mesh_graphadr': ('nmesh',),
             'mesh_normal': ('nmeshnormal', 3),
             'mesh_normaladr': ('nmesh',),
             'mesh_normalnum': ('nmesh',),
             'mesh_octadr': ('nmesh',),
             'mesh_octnum': ('nmesh',),
             'mesh_pathadr': ('nmesh',),
             'mesh_polyadr': ('nmesh',),
             'mesh_polymap': ('nmeshpolymap',),
             'mesh_polymapadr': ('nmeshvert',),
             'mesh_polymapnum': ('nmeshvert',),
             'mesh_polynormal': ('nmeshpoly', 3),
             'mesh_polynum': ('nmesh',),
             'mesh_polyvert': ('nmeshpolyvert',),
             'mesh_polyvertadr': ('nmeshpoly',),
             'mesh_polyvertnum': ('nmeshpoly',),
             'mesh_pos': ('nmesh', 3),
             'mesh_quat': ('nmesh', 4),
             'mesh_scale': ('nmesh', 3),
             'mesh_texcoord': ('nmeshtexcoord', 2),
             'mesh_texcoordadr': ('nmesh',),
             'mesh_texcoordnum': ('nmesh',),
             'mesh_vert': ('nmeshvert', 3),
             'mesh_vertadr': ('nmesh',),
             'mesh_vertnum': ('nmesh',),
             'name_actuatoradr': ('nu',),
             'name_bodyadr': ('nbody',),
             'name_camadr': ('ncam',),
             'name_eqadr': ('neq',),
             'name_excludeadr': ('nexclude',),
             'name_flexadr': ('nflex',),
             'name_geomadr': ('ngeom',),
             'name_hfieldadr': ('nhfield',),
             'name_jntadr': ('njnt',),
             'name_keyadr': ('nkey',),
             'name_lightadr': ('nlight',),
             'name_matadr': ('nmat',),
             'name_meshadr': ('nmesh',),
             'name_numericadr': ('nnumeric',),
             'name_pairadr': ('npair',),
             'name_pluginadr': ('nplugin',),
             'name_sensoradr': ('nsensor',),
             'name_siteadr': ('nsite',),
             'name_skinadr': ('nskin',),
             'name_tendonadr': ('ntendon',),
             'name_texadr': ('ntex',),
             'name_textadr': ('ntext',),
             'name_tupleadr': ('ntuple',),
             'names': ('nnames',),
             'names_map': ('nnames_map',),
             'numeric_adr': ('nnumeric',),
             'numeric_data': ('nnumericdata',),
             'numeric_size': ('nnumeric',),
             'oct_aabb': ('noct', 6),
             'oct_child': ('noct', 8),
             'oct_coeff': ('noct', 8),
             'oct_depth': ('noct',),
             'pair_dim': ('npair',),
             'pair_friction': ('npair', 5),
             'pair_gap': ('npair',),
             'pair_geom1': ('npair',),
             'pair_geom2': ('npair',),
             'pair_margin': ('npair',),
             'pair_signature': ('npair',),
             'pair_solimp': ('npair', 5),
             'pair_solref': ('npair', 2),
             'pair_solreffriction': ('npair', 2),
             'paths': ('npaths',),
             'plugin': ('nplugin',),
             'plugin_attr': ('npluginattr',),
             'plugin_attradr': ('nplugin',),
             'plugin_stateadr': ('nplugin',),
             'plugin_statenum': ('nplugin',),
             'qpos0': ('nq',),
             'qpos_spring': ('nq',),
             'sensor_adr': ('nsensor',),
             'sensor_cutoff': ('nsensor',),
             'sensor_datatype': ('nsensor',),
             'sensor_delay': ('nsensor',),
             'sensor_dim': ('nsensor',),
             'sensor_history': ('nsensor', 2),
             'sensor_historyadr': ('nsensor',),
             'sensor_interval': ('nsensor', 2),
             'sensor_intprm': ('nsensor', 3),
             'sensor_needstage': ('nsensor',),
             'sensor_noise': ('nsensor',),
             'sensor_objid': ('nsensor',),
             'sensor_objtype': ('nsensor',),
             'sensor_plugin': ('nsensor',),
             'sensor_refid': ('nsensor',),
             'sensor_reftype': ('nsensor',),
             'sensor_type': ('nsensor',),
             'sensor_user': ('nsensor', 'nuser_sensor'),
             'site_bodyid': ('nsite',),
             'site_group': ('nsite',),
             'site_matid': ('nsite',),
             'site_pos': ('nsite', 3),
             'site_quat': ('nsite', 4),
             'site_rgba': ('nsite', 4),
             'site_sameframe': ('nsite',),
             'site_size': ('nsite', 3),
             'site_type': ('nsite',),
             'site_user': ('nsite', 'nuser_site'),
             'skin_boneadr': ('nskin',),
             'skin_bonebindpos': ('nskinbone', 3),
             'skin_bonebindquat': ('nskinbone', 4),
             'skin_bonebodyid': ('nskinbone',),
             'skin_bonenum': ('nskin',),
             'skin_bonevertadr': ('nskinbone',),
             'skin_bonevertid': ('nskinbonevert',),
             'skin_bonevertnum': ('nskinbone',),
             'skin_bonevertweight': ('nskinbonevert',),
             'skin_face': ('nskinface', 3),
             'skin_faceadr': ('nskin',),
             'skin_facenum': ('nskin',),
             'skin_group': ('nskin',),
             'skin_inflate': ('nskin',),
             'skin_matid': ('nskin',),
             'skin_pathadr': ('nskin',),
             'skin_rgba': ('nskin', 4),
             'skin_texcoord': ('nskintexvert', 2),
             'skin_texcoordadr': ('nskin',),
             'skin_vert': ('nskinvert', 3),
             'skin_vertadr': ('nskin',),
             'skin_vertnum': ('nskin',),
             'ten_J_colind': ('nJten',),
             'ten_J_rowadr': ('ntendon',),
             'ten_J_rownnz': ('ntendon',),
             'tendon_actfrclimited': ('ntendon',),
             'tendon_actfrcrange': ('ntendon', 2),
             'tendon_actuatorid': ('ntendon',),
             'tendon_adr': ('ntendon',),
             'tendon_armature': ('ntendon',),
             'tendon_damping': ('ntendon',),
             'tendon_dampingpoly': ('ntendon', 2),
             'tendon_frictionloss': ('ntendon',),
             'tendon_group': ('ntendon',),
             'tendon_invweight0': ('ntendon',),
             'tendon_length0': ('ntendon',),
             'tendon_lengthspring': ('ntendon', 2),
             'tendon_limited': ('ntendon',),
             'tendon_margin': ('ntendon',),
             'tendon_matid': ('ntendon',),
             'tendon_num': ('ntendon',),
             'tendon_range': ('ntendon', 2),
             'tendon_rgba': ('ntendon', 4),
             'tendon_solimp_fri': ('ntendon', 5),
             'tendon_solimp_lim': ('ntendon', 5),
             'tendon_solref_fri': ('ntendon', 2),
             'tendon_solref_lim': ('ntendon', 2),
             'tendon_stiffness': ('ntendon',),
             'tendon_stiffnesspoly': ('ntendon', 2),
             'tendon_treeid': ('ntendon', 2),
             'tendon_treenum': ('ntendon',),
             'tendon_user': ('ntendon', 'nuser_tendon'),
             'tendon_width': ('ntendon',),
             'tex_adr': ('ntex',),
             'tex_colorspace': ('ntex',),
             'tex_data': ('ntexdata',),
             'tex_height': ('ntex',),
             'tex_nchannel': ('ntex',),
             'tex_pathadr': ('ntex',),
             'tex_type': ('ntex',),
             'tex_width': ('ntex',),
             'text_adr': ('ntext',),
             'text_data': ('ntextdata',),
             'text_size': ('ntext',),
             'tree_bodyadr': ('ntree',),
             'tree_bodynum': ('ntree',),
             'tree_dofadr': ('ntree',),
             'tree_dofnum': ('ntree',),
             'tree_sleep_policy': ('ntree',),
             'tuple_adr': ('ntuple',),
             'tuple_objid': ('ntupledata',),
             'tuple_objprm': ('ntupledata',),
             'tuple_objtype': ('ntupledata',),
             'tuple_size': ('ntuple',),
             'wrap_objid': ('nwrap',),
             'wrap_prm': ('nwrap',),
             'wrap_type': ('nwrap',)}}
)
# ----------------------------End of generated code----------------------------
